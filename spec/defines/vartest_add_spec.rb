...
def variable_test(title, key, val, opts = {})
  ...
  opts[:err] ||= nil
  opts[:errmsg] ||= %r{.*}
  opts[:content] ||= %r{^\s*#{opts[:key_str]} = #{opts[:val_str]}\n}
  ...
  context "with #{key} => #{val}" do
    ...
    if opts[:err]
      it do
        expect { is_expected.to contain_concat__fragment("stunnel_connection_#{title}") }.to
        raise_error(opts[:err], opts[:errmsg])
      end
    else

      it do
        is_expected.to contain_concat__fragment("stunnel_connection_#{title}").with_content(opts[:content])
      end
    end
  end
end
