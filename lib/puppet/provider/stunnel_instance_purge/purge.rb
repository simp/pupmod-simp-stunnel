Puppet::Type.type(:stunnel_instance_purge).provide(:purge) do
  ...
  def dirs
    ...
    if @system_services
      matching_catalog_services = @resource.catalog.resources.select do |res|
      ...
      if system_service_names.first && system_service_names.first.split('.')[-1] == 'service'
        matching_catalog_service_names.map! { |n| n + '.service' }
      end
      ...
      if rogue_services && !rogue_services.empty?
        ...
        @state_msg = if @resource[:verbose] || @resource[:noop]
                       %(Purged Services: '#{@to_purge.join("', '")}')
                     else
                       %(Purged '#{@to_purge.count}' Services)
                     end
      end
    end
